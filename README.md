...
Although there are many possibilities e.g., Anaconda and Linuxbrew, to get the pre-built executables up running, the bonus to start from the scratch is that we can always use the latest release as well as the test data. It is expected that the site could eventually be enriched by many practitioners.
